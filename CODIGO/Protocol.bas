...
On Error Resume Next
    ...
    Select Case incomingData.PeekByte()
        ...
        Case ServerPacketID.CharParticleCreate
...
'***************************************************
    ...
    Call InvOfferComUsu(0).Initialize(DirectD3D, frmComerciarUsu.picInvOfertaProp, INV_OFFER_SLOTS)
    Call InvOfferComUsu(1).Initialize(DirectD3D, frmComerciarUsu.picInvOfertaOtro, INV_OFFER_SLOTS)
    Call InvOroComUsu(0).Initialize(DirectD3D, frmComerciarUsu.picInvOroProp, INV_GOLD_SLOTS, , TilePixelWidth * 2, TilePixelHeight, TilePixelWidth / 2)
    Call InvOroComUsu(1).Initialize(DirectD3D, frmComerciarUsu.picInvOroOfertaProp, INV_GOLD_SLOTS, , TilePixelWidth * 2, TilePixelHeight, TilePixelWidth / 2)
    Call InvOroComUsu(2).Initialize(DirectD3D, frmComerciarUsu.picInvOroOfertaOtro, INV_GOLD_SLOTS, , TilePixelWidth * 2, TilePixelHeight, TilePixelWidth / 2)
...
Private Sub HandleCharParticle()
    ...
    If Not Particula < 1 And Not Particula > TotalStreams Then
        Call General_Char_Particle_Create(Particula, CharIndex)
    End If
End Sub
...
On Error GoTo ErrHandler
    ...
    If frmBancoObj.Visible Then
        With Inventario
            Call InvBanco(1).SetItem(slot, .OBJIndex(slot), .Amount(slot), _
                .Equipped(slot), .GrhIndex(slot), .OBJType(slot), .MaxHit(slot), _
                .MinHit(slot), .MaxDef(slot), .MinDef(slot), .Valor(slot), .ItemName(slot))
        End With
